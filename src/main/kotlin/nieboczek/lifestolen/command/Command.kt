...
class Command(val name: String) {
    ...
    class Ctx(val arguments: List<Argument>) {
        private fun getArg(name: String): Argument {
            val arg = arguments.find { it.name == name }
            if (arg == null) throw CommandError("Argument $name was not supplied")
            return arg
        }
        ...
        fun getModule(name: String): Module {
            ...
            if (module == null) throw CommandError("No module with ID: $argVal")
            return module
        }
    }
}
